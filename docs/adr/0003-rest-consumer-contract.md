...
- **TTS 於 #46 開始對 `input` 做口語展開**（無形狀變更，但**同一個 `input` 的輸出音訊會改變**）：數字、金額、單位、日期、時刻、電話等書面形式改寫為台灣人會唸的形式後才進模型。消費端不需改任何程式碼，但若它自己已經先做過一次展開（例如把 `3kg` 寫成「三公斤」再送來），那份工作現在是多餘的。長度上限的語義同時明確化為「量在展開之前」——展開不改變語音長度，故不計入額度。清單與邊界情況見 `docs/api/tts.md` §5.1 與 §7。
