...
- TTS 文字前處理層（#46 已落地）：任意台灣繁中送進模型前經 `中性化 {} → 判空與長度 → TN（口語展開）→ G2P（只鎖台陸差異字，輸出 {pinyin+聲調}）`，**順序不可倒置**——TN 會把 `{ni3}` 展開成 `{ni三}`，標記全毀。該層放在 `TtsClient` 之上：純函式、無 I/O、不載入模型，放進 adapter 會使其無法在不接模型的情況下單測。
  ...
  - **但 adapter 內有 OpenCC t2s 那一步**（#51，與上一點方向相反、理由也不同）：VoxCPM2 對特定繁體字會落到**粵語**發音（同句同音色，繁體 7/8 錯、簡體 0/8；ASR 回讀出現啲、嘢、嘅、喺、哋），字形是它判斷語言的訊號。轉換因此放在 `VllmOmniTtsClient` 而非本層——它是這個模型的輸入格式要求，換一個引擎就不需要，而 TN 與讀音鎖定是產品語義。順序為 `TN → 讀音鎖定 → t2s`：標記是 ASCII，轉換動不到它。代價是管理平面的預覽停在前處理層、不再逐字等於送出的字串（US 3 要操作者讀得懂那段文字）。
