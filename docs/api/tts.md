...
**口語展開不計入長度。** 上限是語音長度的代理值，而展開不改變語音長度——`NT$1,250` 與「新臺幣一千二百五十元」唸起來一樣長，字元數卻從 8 變成 10。所以你可以用自己算得出來的字元數預測 413，不必猜展開會膨脹多少。

**`input` 是純文字，控制標記由 BFF 全權處理。**

**口語展開已實作，台灣破音字鎖定尚未。**（此處的「展開」即業界所稱的 text normalization；本文不用「正規化」一詞，那在本專案的 ASR 側指音檔轉碼。）
...
**讀音鎖定：只涵蓋一份策展清單。** VoxCPM2 沒有台灣國語的訓練目標，兩岸讀音不同的字會落到 zh-CN 的讀法（「垃圾」唸 lā jī、「品質」唸 pǐn zhì、「期限」唸 qī xiàn）。BFF 對清單內的字詞注入讀音標記矯正，選字準則是「會出現在保險業務對話裡」。

**清單外的字仍是模型的預設讀法**，這不會回錯誤，只會唸錯。表刻意小：鎖一個本來就唸對的字是我方新增的錯誤，不鎖只是保留原狀，兩者的代價不對稱。
