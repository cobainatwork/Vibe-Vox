...
| `type` | `"clone"` \| `"design"` | 建立方式，**僅供顯示與分組**。對合成行為無影響，兩型能力相同 |
| `language` | 字串 | BCP 47，例如 `zh-TW`。音色建立時指定，用於挑選適配的音色 |

**以 `id` 綁定音色，不要以 `name` 綁定。** `name` 是給人看的，操作者隨時會改。

`type` 只反映音色怎麼建立的（上傳參考音為 `clone`、文字描述生成為 `design`），**不代表能力差異**。兩型都支援 `instruct`。
...
| `instruct` | 字串 | 否 | — | Instruction：控制語氣、情緒、韻律的自然語言指示 |
...
一次請求只承載一種語氣。若一段話中間要換情緒，就切成多次請求，各自帶不同的 `instruct`。

範例值：`溫和、語速偏慢`、`不耐煩、打斷對方`、`猶豫，句尾上揚`。
...
```json
[
  { "input": "喔，這樣啊。",           "instruct": "遲疑" },
  { "input": "那費用大概是多少？",     "instruct": "轉為關注，語速加快" }
]
```
