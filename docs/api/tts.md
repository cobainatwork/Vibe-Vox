...
現階段三個缺口，各自在下文標註：**分塊串流**（§5.4）與 **mp3 輸出**（§5.3）帶了會回 400；**台灣破音字的讀音鎖定**（§5.1）則是靜默的品質落差，那是唯一需要 provider 現在就知道的。書面→口語的展開（數字、金額、單位、日期）已實作，清單見 §5.1。
...
**尚未實作：台灣破音字鎖定。** 「垃圾」「企業」「星期」「他和我」這類兩岸讀音不同的字，目前落到模型自己的預設，而那是 zh-CN 的讀法。這不會回錯誤，只會唸錯。追蹤於 #46。
