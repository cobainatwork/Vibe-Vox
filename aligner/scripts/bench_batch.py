...
**已知的量測偏差**：下方送出的是同一段音訊重複 batch_size 次，故批次內段長完全均勻。
批次張量會 pad 到該批最長的段落，均勻輸入的 padding 浪費恰好 1.00 倍，而真實錄音的段長
不均（實測 1.77 至 41.29 秒），padding 後接近實際音訊的 2 倍。**本腳本因此系統性低估真實
負載**，其數字曾使 batch 上限被定在 32 而在真機 OOM（#36）。要量真實負載須改為送入不同
長度的段落，取該批最長者計算 padding 後的總量。
