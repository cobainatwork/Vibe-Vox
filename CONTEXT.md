...
**唸錯分兩類，成因與解法都不同。** 一是**兩岸讀音不同**（台灣 lè sè 對大陸 lā jī），字級固定，可從讀音表機器導出。二是**兩岸都是破音字而模型選錯那一讀**（「倒垃圾」的 `倒` 該唸 dào，模型唸 dǎo），同一個字在不同上下文有不同答案，只能靠上下文判斷。

**只鎖一份策展清單，不全句轉拼音。** 鎖一個本來就唸對的字是我方新增的錯誤，不鎖只是保留模型的預設——兩者的代價不對稱，這條不對稱決定了清單的所有取捨。清單外的字維持模型的預設讀法。
