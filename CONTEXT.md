...
把兩岸讀音不同的字詞，就地換成 VoxCPM2 的 `{pinyin+聲調}` 讀音標記的動作（「垃圾」→ `{le4}{se4}`）。是口語形式的一部分。

**只鎖一份策展清單，不全句轉拼音。** 鎖一個本來就唸對的字是我方新增的錯誤，不鎖只是保留模型的預設——兩者的代價不對稱，這條不對稱決定了清單的所有取捨。清單外的字維持模型的 zh-CN 讀法。
_Avoid_: 破音字處理（清單內多數是台灣單音字，只是與大陸不同）、G2P（縮寫不進使用者看得到的文字）、注音標記（送出去的是拼音）
