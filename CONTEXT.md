...
**口語形式（Spoken form）**：
一段文字經前處理層之後的樣子。書面形式在此改寫為台灣人會唸的形式：`3kg` → 三公斤、`NT$1,250` → 新臺幣一千二百五十元、`2026/8/5` → 二零二六年八月五日、`25℃` → 攝氏二十五度。改寫在 `TtsClient` 之上完成，模型端不做正規化。

書面改寫成口語的那個動作稱「口語展開」，把讀音換成標記的動作稱「讀音鎖定」（見下條）。

**它不等於送進模型的字串。** adapter 之下還有一層字形轉簡（見「字形轉簡」條），而那一層不屬於「台灣人會怎麼唸」。

**唸錯不回錯誤碼、不進 log**，故管理平面把這個字串顯示出來——操作者要能分辨是改寫錯了還是模型錯了。顯示的是這個繁體字串而非送出去的那個：他要讀得懂才做得了那個判斷。
_Avoid_: 正規化（該詞在 ASR 側指音檔轉碼）、TN（縮寫不進使用者看得到的文字）、口語化、送進模型的文字（那是轉簡之後的東西）

**讀音鎖定（Pronunciation locking）**：
把模型會唸錯的字詞，就地換成 VoxCPM2 的 `{pinyin+聲調}` 讀音標記的動作（「垃圾」→ `{le4}{se4}`）。是口語形式的一部分。

**唸錯分三類，成因與解法都不同。** 一是**上下文改變讀音**（「銀行」的 `行` 該唸 háng 而非它單獨出現時的 xíng），由判準自動挑出，不必維護任何清單。二是**兩岸讀音不同**（台灣 lè sè 對大陸 lā jī），字級固定，可從讀音表機器導出。三是**兩岸都是破音字而模型選錯那一讀**（「倒垃圾」的 `倒` 該唸 dào，模型唸 dǎo），沒有任何工具能事先預測模型會在哪裡選錯，只能實測撞到才逐筆加。

**不全句轉拼音。** 鎖一個本來就唸對的字是我方新增的錯誤，不鎖只是保留模型的預設——兩者的代價不對稱，這條不對稱決定了三類的所有取捨，也是「寧可漏鎖，不可誤鎖」這句話的來源。真實逐字稿上實際被鎖的是 2,305 個漢字裡的 29 個。
_Avoid_: G2P（縮寫不進使用者看得到的文字）、注音標記（送出去的是拼音）

**判準（Criterion）**：
決定某個字要不要被鎖讀音的規則，形式是「它在詞組裡的讀音」與「它單獨出現時的預設讀音」相不相同——只有上下文真的改變了讀音才鎖。

**它取代的是手打的前後文清單。** 那種清單每一份都是編輯判斷、沒有可查證的真值，而錯的方式是靜默的；中文常用破音字有幾十個，照那條路每個字都要一份清單。判準讓「新增一個破音字」不必編輯任何東西。

**它有涵蓋不到的類**：模型預設就唸錯的字（上一條的第三類）在判準眼裡兩個讀音相同，看不到。
_Avoid_: 破音字集合（判準刻意不問「這個字有幾種讀法」）、消歧模型（沒有模型，是純函式）

**字形轉簡（Simplification）**：
送進 TTS 模型之前把繁體字形換成簡體的動作，在 adapter 內、口語形式與讀音鎖定都完成之後。

**它處理的不是「這個字唸哪個音」而是「模型用哪種語言在唸」。** VoxCPM2 對特定繁體字會落到粵語發音——ASR 回讀出現粵語專用字（啲、嘢、嘅、喺、哋），同一句同一音色，繁體 7/8 錯、簡體 0/8。字形是模型判斷語言的訊號，推測成因是訓練語料裡繁體字的主要來源是粵語。

**它與 ASR 側的字形轉繁是同一件事的兩半**，兩者都是模型端的格式要求而非產品語義：換掉任一端的模型，對應的那一半就不需要了。因此放在 adapter 而非前處理層。

**代價是管理平面的預覽不再逐字等於送出的字串**（見「口語形式」條）。
_Avoid_: 簡繁轉換（方向要明講）、正規化（該詞在 ASR 側指音檔轉碼）、OpenCC（那是工具名不是概念）
...
**參考音（Reference audio）**：
決定某個 Voice 聽起來是誰的那一段音檔，是該音色身分的唯一錨點。clone 為使用者上傳、design 為定版擷取的生成音，兩型在合成路徑上完全相同。

**它的可用性是 Voice 的不變量**：存在、可解碼、時長在模型端強制的 1.0 至 30.0 秒內。這三件事在建立音色時判定一次，之後的合成假設它成立——放在合成路徑等於每次請求都算一遍，而那時已經沒有人能修正它。不變量之前建立的音色不受涵蓋，故音色清單逐列回報不可用的原因，合成則回 409 `VOICE_UNUSABLE`。
_Avoid_: 樣本音、聲音樣本、逐字稿（`ref_text` 是另一回事，且不進合成路徑）
