...
**口語形式（Spoken form）**：
一段文字實際會被送進 TTS 模型的樣子。書面形式在此改寫為台灣人會唸的形式：`3kg` → 三公斤、`NT$1,250` → 新臺幣一千二百五十元、`2026/8/5` → 二零二六年八月五日、`25℃` → 攝氏二十五度。改寫在 `TtsClient` 之上完成，模型端不做正規化。

書面改寫成口語的那個動作稱「口語展開」，把讀音換成標記的動作稱「讀音鎖定」（見下條）。

**唸錯不回錯誤碼、不進 log**，故管理平面把這個字串顯示出來——操作者要能分辨是改寫錯了還是模型錯了。
_Avoid_: 正規化（該詞在 ASR 側指音檔轉碼）、TN（縮寫不進使用者看得到的文字）、口語化
...
**不全句轉拼音。** 鎖一個本來就唸對的字是我方新增的錯誤，不鎖只是保留模型的預設——兩者的代價不對稱，這條不對稱決定了三類的所有取捨，也是「寧可漏鎖，不可誤鎖」這句話的來源。真實逐字稿上實際被鎖的是 2,305 個漢字裡的 28 個。
