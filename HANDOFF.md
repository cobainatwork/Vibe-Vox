...
**日期**：2026-08-04
**分支**：main，與 `origin/main` 同步於 `e6815ac`（#35 的修正尚未 merge，見第 3 節）
**範圍**：字級強制對齊 T2–T4 落地（#27 #28 #29，該條線全數完成）、Hotword 持久化缺陷修正（#33）、對齊判準過嚴的修正（#34）、逾時鏈路修正（#35）
...
期間修掉三個會讓功能實質不可用的缺陷，**三者都是使用者實測才發現的，不是測試抓到的**：Hotword 每次 rebuild 就消失（#33）、合理性檢查對長段落過嚴使 9 段錄音只有 2 段通過（#34，修正後同一份錄音全段通過）、nginx 逾時從未設定使逾 5 分鐘的音檔必然 504（#35）。
...
| 位置 | 票 | 內容 |
|---|---|---|
| 分支 `fix/asr-timeout-chain` | #35 | nginx 逾時未設，逾 5 分鐘的音檔必然 504 |
...
**#35 尚未 merge**，在分支 `fix/asr-timeout-chain` 上；其餘皆已 push。

**測試**：`bff` 128 passed／4 skipped、`aligner` 20 passed、`frontend` typecheck + 34 passed + production build。

期間開了四張票：**#32**（四個推算閾值待校準）、**#33**、**#34**（皆已修並關閉）、**#35**（已修，待 merge）。
