...
**接手的第一件事：部署後重跑那份會議錄音，確認單字時長下界的修正救回多少段。** #36 的分批已在真機驗收通過（batch 上限降為 8 之後），但修正過程中發現的浮點假陽性剛修完、還沒實跑。預期通過段數由 36 升至約 54，見 4.1 與 #32。

**#32 的校準路徑已經打開。** 那份錄音現在能對齊，36 段、1941 字的實測資料已寫進 #32，並據此修掉一個把 216 個正常字誤判為異常的缺陷。
...
### 2.3 `VIBE_VOX_ALIGNER_MAX_BATCH_ITEMS = 8`：不要調高

**8 是實測值，取代原本校準出的 32**（32 在真機上 OOM，見 2.2）。日常負載 2 至 4 段，觸不到它；會撞到的只有管理平面的長音檔測試，而那條路多幾次往返只差幾秒。調高沒有收益，只是把 aligner 往同一個天花板推。
...
真正的容量問題是 vLLM 佔 44.40 GiB 中的 37.71 GiB（85%）且 `gpu_memory_utilization` 從未設定過，追蹤於 #31。
...
**已在真機驗收，但過程中 batch 上限被實測推翻。** 分批本身有效，然而 cap 32 在真機 CUDA OOM（兩批都失敗），降為 8 後才通過：57 段送出、36 段對齊、1941 字。錄音在 `D:\SAP\華厚線上發票問題討論-20250521_141801-會議錄製_0~10.wav`，OOM 的完整分析見 2.2。
