...
**日期**：2026-08-04
**分支**：main，與 `origin/main` 同步於 `63c3411`（工作樹乾淨，本 session 全部已 merge 並 push）
**範圍**：字級強制對齊 T2–T4 落地（#27 #28 #29）、Hotword 持久化（#33）、對齊判準過嚴（#34）、逾時鏈路（#35）。另開三張未動工的票（#36 #37 #38），見第 4 節
...
期間修掉三個會讓功能實質不可用的缺陷，**三者都是使用者實測才發現的，測試一個都沒抓到**：Hotword 每次 rebuild 就消失（#33）、合理性檢查對長段落過嚴使 9 段錄音只有 2 段通過（#34，修正後同一份錄音全段通過）、nginx 逾時從未設定使逾 5 分鐘的音檔必然 504（#35）。

**接手的第一件事：#36。** 會議錄音（63 段）超過 batch 上限而 adapter 未實作分批，導致全段無法對齊。它同時擋住 #32 的校準，而那是目前唯一能推進的校準路徑。

**#36 之後，最大的未動工項目是 TTS 引擎變更**（#13 map + #14–#20，共十張票）：它同時解掉三份文件與已定案決策的不一致、解封 #6–#8、也讓 #31 有辦法往下走。兩者不衝突：#36 是一天內的修正，TTS 那條線是數天的工作。

#32（閾值校準）在 #36 修完後才動得了，且它需要的資料使用者可以自己錄，不必等接上 AI_practise，理由見 8.1.1。
...
**測試**：`bff` 128 passed／4 skipped、`aligner` 20 passed、`frontend` typecheck + 36 passed + production build。nginx 配置經 `nginx -t` 驗證。

開了七張票：**#32**（四個推算閾值待校準）、**#33 #34 #35**（皆已修並關閉）、**#36 #37 #38**（未動工，見 4.1）。
...
有兩條線。**4.1 是阻塞的、範圍小、有現成的重現資料；4.2 是最大的未動工項目。**

### 4.1 #36 #37 #38：會議錄音揭露的三個缺陷

由使用者實測 10 分鐘、五語者的會議錄音發現，三張票都動 `bff/src/vibe_vox/adapters/aligner.py`，**建議一輪做完**（同一個檔案、同一批測試與審查）。

**#36（阻塞）**：63 個 Segment 全部送去對齊，超過 `MAX_BATCH_ITEMS = 32`，aligner 回 400 `BATCH_TOO_LARGE`，adapter 映射為 `AlignerUnavailable`，端點層降級成全段未對齊。
...
**這推翻了 #27 的一個假設。** 不分批的理由寫在 `aligner/README.md`：「日常負載 2–4 段，一次送完即可」，那是單語者回合制對話的假設。多語者會議的切分完全不同：語者切換處必然切段，610 秒切出 63 段、平均 9.65 秒（ADR-0004 記載的是 30 至 40 秒），且有大量「是啊，是啊。」這類 0.77 至 1.6 秒的應答段。

同一份 README 還寫著「若真遇到超過 32 段的輸入，分批的代價也很小」。**當時知道會發生、也判斷了代價很小，但沒有實作。** 而 aligner 的錯誤訊息本身就是「請分批送」，呼叫端卻沒有那個能力。

修法是 `align()` 分批（63 段切成 32 + 31，每批 VRAM 不超過已實測的 5750 MiB，耗時約 2.8 秒）。**不要調大 `MAX_BATCH_ITEMS`**：32 段實測 5750 MiB，64 段推估 9218 MiB 超出可用量。分批後須維持批次級的故障隔離：某批失敗不該丟棄其他批的結果。
...
診斷 #36 花的工夫本來不必要。**答案完整寫在 aligner 的回應體裡**（4.1 有原文），而 adapter 的 `raise AlignerUnavailable from exc` 把它整份丟棄，log 只印出 `AlignerUnavailable()`。於是我從「63 段」這個數字反推出結論，重新導出了系統早就知道的事。
...
**但先修 #36**：目前手上唯一的真人樣本是 10 分鐘會議錄音，而它因 63 段超過 batch 上限而完全無法對齊。校準拿不到任何資料。
