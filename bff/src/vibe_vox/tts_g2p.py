...
# 「倒」的傾倒義在台灣唸 dào（倒垃圾、倒杯水），跌倒義唸 dǎo（倒閉、跌倒、顛倒）。
# **這不是兩岸差異**——大陸同樣分兩讀——而是模型選錯了那一讀，操作者實際聽到「倒垃圾」
# 被唸成 dǎo（2026-08-08 聽測）。故它不在本模組開頭那段問題陳述的射程內，是另一類缺陷。
#
# **詞級，不是字元級。** 第一版用「後接字白名單加前接動詞排除」，審查實測出七句誤鎖（那七
# 句現在是 `test_the_falling_dao_is_never_locked` 的安全網）。根因是「跌倒義的前接動詞」是
# **開放集合**（撲醉暈栽癱嚇病累撞滑吹…），而封閉列舉蓋不住開放集合——那是把 `_NOT_LOCKED`
# 的「有限例外蓋有限詞」誤用了。
#
# **涵蓋率刻意極低。** 詞級同樣沒有詞界意識（見上一段的第三個原則），所以連 `倒車`、`倒數`
# 都不能收：「他跌倒車上」「他跌倒數次」都是自然句，兩字詞照樣跨界。只收三字且前面接任何
# 跌倒義動詞都語意不通的搭配。`倒水`、`把垃圾倒掉` 因此漏鎖——漏鎖保留模型預設，誤鎖是我方
# 新增的錯音，這個不對稱優先於涵蓋率。
#
# 讀音 dào 取自**教育部辭典的語義區分**，不是 g2pW 表的首選——那份表給的是字級首選讀音，
# 對兩讀都活的字幫不上。這一筆與 `和` 一樣是編輯判斷。
#
# **這是止血。** 任何更寬的清單都被實測出誤鎖，而那正是 #50 要換掉整個機制的理由；換的時候
# 這一份與上面 `和` 的兩份清單一起刪。
#
# 清單寫成完整的詞（而不是只寫後綴），因為那是人要核對的形式；比對時只吃 `倒` 本身，後綴
# 留給主表——`倒垃圾` 的 `垃圾` 仍要走 char 級規則變成 `{le4}{se4}`。
_POUR_DAO_WORDS = ("倒垃圾", "倒杯水", "倒杯茶")
_POUR_DAO = re.compile(
    "倒(?=" + "|".join(re.escape(word[1:]) for word in _POUR_DAO_WORDS) + ")"
)
...
def lock_taiwan_readings(text: str) -> SpeechText:
    ...
    三趟，順序不可換：`倒` 與「和」的判準是前後文而非字串本身，而 `倒` 必須在主表之前——
    主表會把 `倒垃圾` 的 `垃圾` 換成標記，讓「後接哪一個字」這個判準看不到字。
    """
    poured = _POUR_DAO.sub("{dao4}", text)
    locked = _TARGET.sub(_spell, poured)
