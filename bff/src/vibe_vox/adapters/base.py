...
class TtsClient(Protocol):
    ...
    async def synthesize(
        ...
        **reference_audio 的可用性是呼叫端的前置條件**：存在、可讀、容器合法、時長在
        模型端的 1.0–30.0 秒內。它由 Voice 建立時的不變量（`audio/reference.py`）加上
        端點在解析路徑時的檢查（`api/tts.py` 的 `VoiceUnusable` → 409）保證，故實作
        **不需重驗**——驗一次就夠的東西放進每次合成只是重複付費。

        殘餘的情形是那道檢查與實作真的讀檔之間的時間差（DB 還原、volume 換掛、人工
        刪檔）。讀不到時**須翻成 `TtsUnavailable`**，不得讓 `OSError` 逸出：它會穿過
        `HeavyRequestGuard` 冒成 500，而 500 不在 docs/api/tts.md §6 的錯誤表內。
        不讀參考音的實作（stub）沒有這個情形，不需要假裝有。
        """
        ...
