...
#
# **不要為了少幾次往返而調高。** 資料平面是回合制對話的 2–4 段，觸不到任何上限；會
# 撞到的只有管理平面的長音檔測試，而那條路多幾次往返的代價是幾秒。真正的容量問題是
# vLLM 佔了 44.40 GiB 中的 37.71 GiB 且 `gpu_memory_utilization` 從未設定過（#31），
# 調高本值只是把 aligner 往那個天花板推。
