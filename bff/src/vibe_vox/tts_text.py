...
管線順序為中性化 → 判空與長度 → TN → 鎖台灣讀音（見 api/tts.py 的 to_utterance）。
...
class SpeechText(str):
    """已完成前處理、可以原樣送給模型的合成文字。

    **它的存在是為了讓「誰有權發出大括號」成為型別層的事實。** `{}` 是 VoxCPM2 的讀音標記
    語法，使用者文字裡的大括號必須被中性化（見下方 `_NEUTRALIZED`），而 G2P 層注入的標記
    必須原樣送出。少了這個型別，`Utterance` 的 validator 分不出兩者，只能一律轉全形——那會
    把讀音標記一起毀掉。

    是 `str` 的子類而不是包裝型別：它要能直接進 f-string、比長度、送 tokenizer，包一層只會
    讓每個使用點都得記得解包。**代價是 pydantic 對 `str` 欄位會轉掉子類身分**，故 `Utterance`
    以一段 after validator 重新包裝，讓它在值上活著（見 adapters/base.py）。

    產出它的只有兩處：`tts_g2p.lock_taiwan_readings`，與 `Utterance` 的 validator（那裡的
    輸入已經中性化過）。Python 關不掉建構子，所以這是慣例而非強制——但它與
    `Utterance.model_construct` 是同一類逃生口：要繞過得在伺服器端刻意寫一行，使用者輸入
    到不了這裡。
    """
